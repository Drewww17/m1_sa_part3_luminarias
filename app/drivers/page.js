...
export default function DriversPage() {
  ...
  // Initialize season from localStorage on mount
  useEffect(() => {
  ...
  useEffect(() => {
    async function loadDrivers() {
      ...
      try {
        ...
        // If standings are empty, fallback to roster
        if (!standingsData || standingsData.length === 0) {
          console.log(`No standings for season ${season}, fetching roster...`);
          setUsingFallback(true);
          
          const rosterData = await getDriversList(season === 'current' ? '2026' : season);
          
          // Map roster to standings UI shape
          const mappedDrivers = rosterData.drivers.map((driver, index) => ({
            position: (index + 1).toString(),
            points: '0',
            wins: '0',
            Driver: driver,
            Constructors: [] // No constructor info in roster endpoint
          }));
          ...
          // Check if all points are zero and sort alphabetically if so
          if (allZeroPoints(standingsData)) {
            setDrivers(sortDriversAlphabetically(standingsData));
          } else {
