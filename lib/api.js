...
export async function getDriversList(season = 'current') {
  try {
    // If season is 'current', use explicit '2026' for roster fallback
    const actualSeason = season === 'current' ? '2026' : season;
    const url = `${ERGAST_BASE_URL}/${actualSeason}/drivers.json`;
    const data = await fetchWithCache(url);
    ...
    return {
      season: season === 'current' ? '2026' : season,
      drivers: [],
      isMock: true
    };
  }
