...
async function fetchWithCache(url, cacheDuration = CACHE_DURATION) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Fetch failed for ${url}: status ${response.status}, response: ${responseText}`);
...
/**
 * Get drivers list for a season (roster)
 * @param {string} [season='current'] - Season year or 'current'
...
export async function getDriversList(season = 'current') {
  try {
    ...
    if (drivers.length === 0) {
      console.log('No drivers found for season, returning empty roster');
      return { season, drivers: [], isMock: true };
    }
    
    return { season, drivers, isMock: false };
  } catch (error) {
 ...
 * @returns {Promise<import('./types').Constructor | null>}
 */
