...
async function fetchWithCache(url, cacheDuration = CACHE_DURATION) {
  ...
  try {
    ...
    if (!response.ok) {
      // Clone response before consuming body to avoid issues
      const responseClone = response.clone();
      const errorText = await responseClone.text().catch(() => 'Unable to read response');
      console.error(`API request failed: ${response.status} ${response.statusText} - URL: ${url} - Response: ${errorText}`);
...
/**
 * Get drivers list/roster for a season (fallback when standings are empty)
 * @param {string} [season='2026'] - Season year
 * @returns {Promise<{season: string, drivers: Array, isMock: boolean}>}
 */
export async function getDriversList(season = '2026') {
  try {
    ...
    if (driversList.length === 0) {
      console.log(`No drivers found for season ${season}, using mock data`);
      // Convert mock standings to roster format
      const mockRoster = mockDriverStandings.map(standing => ({
      ...
      return { season, drivers: mockRoster, isMock: true };
    }
    
    return { season, drivers: driversList, isMock: false };
  } catch (error) {
    console.error(`Failed to fetch drivers list for season ${season}:`, error);
...
export async function getConstructorDetails(constructorId) {
  try {
    ...
    console.error(`Failed to fetch constructor details for ${constructorId}:`, error);
    return null;
  }
}
