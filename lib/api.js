 ...
 * @returns {Promise<{standings: import('./types').DriverStanding[], isMock: boolean}>}
 */
export async function getDriverStandings(season = 'current') {
  try {
    ...
    if (standings.length === 0) {
      ...
      return { standings: mockDriverStandings, isMock: true };
    }
    
    return { standings, isMock: false };
  } catch (error) {
 ...
 * @param {string} [season='current'] - Season year or 'current'
 * @returns {Promise<{standings: import('./types').ConstructorStanding[], isMock: boolean}>}
 */
export async function getConstructorStandings(season = 'current') {
  try {
    ...
    if (standings.length === 0) {
      ...
      return { standings: mockConstructorStandings, isMock: true };
    }
...
/**
 * Get drivers list (roster) for a specific season
 * @param {string} [season='current'] - Season year or 'current'
...
export async function getDriversList(season = 'current') {
  try {
    ...
    if (drivers.length === 0) {
      console.log('Using mock drivers list data');
      // Return mock data structured as roster
      return {
        season,
        drivers: mockDriverStandings.map(d => d.Driver),
        isMock: true
      };
    }
    
    // Convert drivers list to standings format for UI compatibility
    ...
    return {
      season,
      drivers: mockDriverStandings.map(d => ({...d, points: '0', wins: '0'})),
      isMock: true
    };
  }
}
...
export async function getConstructorDetails(constructorId) {
  try {
    ...
    if (!constructor) {
      console.log(`No constructor found for ${constructorId}`);
      return null;
    }
